const bossCityEntries: [string, string][] = [
  ["鞍山", "101070300"],
  ["阿拉善盟", "101081200"],
  ["安康", "101110700"],
  ["阿克苏地区", "101131000"],
...
const bossCityNameToCode = new Map(bossCityEntries)

export function resolveBossCityCode(city: string): string {
  const trimmed = city.trim()
  if (!trimmed) return trimmed
  if (/^\d{6,}$/.test(trimmed)) return trimmed
  const code = bossCityNameToCode.get(trimmed)
  if (code) return code
  return trimmed
}
