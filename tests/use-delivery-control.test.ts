...
type DeliveryControlQueryOptions = {
  ...
  ensureStoresLoaded: () => Promise<void>
  ensureSupportedPage: () => boolean
  fail: (code: string, message: string) => Promise<unknown>
  ok: (code: string, message: string) => Promise<unknown>
}
...
const deliveryControlMocks = vi.hoisted(() => ({
  ...
  createBossHelperAgentResponse: vi.fn((ok: boolean, code: string, message: string, data?: unknown) => ({
    ok,
    code,
    message,
    data,
