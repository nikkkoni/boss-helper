...
describe('request', () => {
  ...
  it('maps background abort-like errors into timeout messages', async () => {
    ...
    await expect(
      request.get({ isBackground: true, url: 'https://example.com/bg-timeout' }),
    ).rejects.toEqual(
      expect.objectContaining({
        message: '请求超时',
        name: '请求错误',
  ...
  it('delegates background requests to counter.request and extracts status codes from errors', async () => {
    ...
    await expect(
      request.get({ isBackground: true, url: 'https://example.com/bg' }),
    ).rejects.toEqual(
      expect.objectContaining({
  ...
  it('returns background responses unchanged when the bridge succeeds', async () => {
    ...
    await expect(
      request.post({ isBackground: true, url: 'https://example.com/bg-ok' }),
    ).resolves.toEqual({ bridge: true })
  })
  ...
  it('maps background string errors into RequestError instances', async () => {
    ...
    await expect(
      request.get({ isBackground: true, url: 'https://example.com/bg-string-error' }),
    ).rejects.toEqual(
      expect.objectContaining({
  ...
  it('keeps explicit RequestError instances intact', async () => {
    ...
    await expect(
      request.get({ isBackground: true, url: 'https://example.com/custom' }),
