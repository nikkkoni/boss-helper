...
- 投递成功后的自定义打招呼和 AI 打招呼
- AI 筛选、外部 AI 审核闭环
- 运行统计、结构化日志、风险摘要和 run checkpoint
- CLI、bridge、MCP、外部 Agent 自动化接入

AI 打招呼是独立开关，不需要同时开启自定义招呼语；如果两者同时开启，投递成功后会优先发送 AI 生成内容。
...
- Boss 直聘网页接口
- Boss 直聘网页聊天 SDK：仅在启用自定义或 AI 打招呼后按页面侧逻辑加载
- OpenAI 兼容接口：仅在你配置自己的模型与 API Key，并启用 AI 筛选或 AI 打招呼后使用
