...
- `boss_helper_navigate` -> `navigate`；当当前 Boss tab 漂移到站内但不受支持的页面时，也可直接用它把现有 tab 拉回受支持职位页
...
- `workflow`：把 `agent-orchestrator` 中已经验证过的高层流程判断结构化下沉到 MCP，包括当前所处 `stage`、当前目标 `goal`、判断原因 `why`、下一组建议动作 `nextActions`、建议优先调用的 `recommendedTools`，以及在事件驱动阶段补充的 `eventFocus.watchTypes` / `eventFocus.terminalTypes`、在岗位分析阶段补充的 `candidateFocus`、在已读取 preview 时补充的 `planFocus`
...
相比 `recommendations` 主要提供自然语言恢复建议，`workflow` 更适合做稳定分支：

- 当 `stage=review-loop` 时，优先处理 `job-pending-review -> boss_helper_jobs_review`
- 当 `stage=observe-run` 时，优先观察 `stats` / `events_recent` / `wait_for_event`
- 当 `stage=resume-run` 时，先复核 `risk` 与 `lastError`，再决定 `resume` 还是 `stop`
- 当 `stage=analyze-jobs` 时，先走 `jobs.current` / `jobs.list` / `jobs.detail` / `resume.get` / `plan.preview`

其中 `workflow.eventFocus` 只在“先观察事件再决策”比继续扩散执行范围更重要的阶段才会非空：

- `watchTypes`：当前阶段建议优先订阅或轮询的事件类型
- `terminalTypes`：出现后通常意味着应结束本轮观察、改做恢复或收尾决策的事件类型

其中 `workflow.candidateFocus` 只在 `stage=analyze-jobs` 这类“先做小范围岗位分析”的阶段才会非空：

- `inspectFirst`：建议优先读取详情或做 `plan.preview` 的少量岗位快照；当前会优先返回列表里已经有 `hasCard=true` 的岗位，再按当前列表顺序补齐
- `loadedCardCount`：当前列表里已有详情卡片的岗位数
- `visibleCount`：当前参与本轮候选判断的可见岗位数

`boss_helper_agent_context` 现在也支持可选 `include=['plan']`：它会在聚合结果 `sections.plan` 中直接透传 `boss_helper_plan_preview` 的只读返回，避免外部 Agent 先调一次 `agent_context`、再单独做第二次聚合才能把 preview summary 合回当前上下文。为了降低真实页面上的扰动，这里的聚合 preview 会默认先收敛范围：优先使用当前已选中岗位；如果当前没有已选中岗位，再退回 `candidateFocus.inspectFirst` 的少量候选岗位，而不是直接对整页列表做 full preview。为避免调用方继续猜这次 preview 是怎样收敛出来的，`sections.plan.scope` 现在会显式返回本次聚合 preview 的来源和命中岗位；当前最小枚举包括 `selected-current-job`、`candidate-focus` 和 `page-default`。

当 `sections.plan` 已存在时，`workflow.planFocus` 会把 preview 结果再压成更高层的下一步提示：

- `firstAction`：当前应优先收敛到哪条高层分支，例如 `narrow-to-ready`、`handle-external-review`、`inspect-manual-review`、`fill-missing-info`
- `inspectFirst`：优先复核的前几个 preview 项，只保留 `decision`、`encryptJobId`、`jobName`、`stage`
- `scope`：复用 `sections.plan.scope` 的收敛来源和命中 `targetJobIds`，让外部 Agent 明确知道当前 preview 是命中已选中岗位，还是回退自 `candidateFocus`
- `summary`：复用 preview summary 的关键计数，避免外部 Agent 重新人工统计

这就是 Phase 10 当前最小增量的边界：不是新增另一套高风险 workflow tool，而是把已验证的 orchestrator 判断下沉进 `boss_helper_agent_context`，让外部 Agent 不用自己复制整套编排规则。

同时要区分两条语义边界：`boss_helper_agent_context(include=['plan'])` 的目标是低扰动地把 preview 结果并回当前上下文，所以它会主动缩小默认预演范围；而 direct `boss_helper_plan_preview` 仍保持独立 tool 的显式语义，是否传 `jobIds` 以及是否对整页做 preview 仍由调用方自己决定。
...
- `failure-count-auto-stop`：当前批次累计失败已达到阈值，本轮已自动进入暂停收尾
- `run-delivery-limit-nearby`：当前 run 已接近每轮投递上限，继续执行前应确认是否真的还要扩容本轮规模
- `run-delivery-limit-reached`：当前 run 已达到每轮投递上限，需先 stop 当前 run，再 start 新一轮
...
如果你只想判断“现在页面能不能操作”，读 `boss_helper_agent_context.readiness` 就够了；如果你只是想把一个小范围 preview 结果并回当前上下文，可用 `boss_helper_agent_context(include=['plan'])`；如果你要明确决定“现在 start 会处理哪些岗位以及为什么”，尤其是需要自己控制 `jobIds` 或整页范围时，则优先直接调用 `boss_helper_plan_preview`。
...
- `suggestedAction=navigate`：先切回受支持的职位搜索页；如果当前只是停在 `verify.html`、Boss 首页或其他 unsupported Boss 页面，`boss_helper_navigate` 现在会直接复用当前 Boss tab 做恢复，不必先等页面 controller 就绪
...
`boss_helper_config_update` 现在也加入了更细粒度的聊天自动化护栏：如果 `configPatch` 会启用 `aiReply`，或修改一个已经启用的 `aiReply` 配置，也必须显式传 `confirmHighRisk=true`，否则会返回同样的 `code=high-risk-action-confirmation-required` / `suggestedAction=fix-input`。推荐做法是先读 `boss_helper_stats.data.risk` 或 `boss_helper_agent_context.summary`，确认当前风险摘要后，再决定是否真的要放开 AI 自动回复。
...
其中 `boss_helper_start`、`boss_helper_resume`、`boss_helper_chat_send` 都属于显式高风险动作；`boss_helper_config_update` 在 patch 涉及启用或修改已启用的 `aiReply` 时也属于同一类高风险写操作。上述场景都必须在 payload 中传 `confirmHighRisk=true`，否则 MCP 会直接拒绝执行。

如果你希望利用 `boss_helper_start` / `boss_helper_resume` 的阻断响应只读查看这份执行前摘要，可以显式传 `confirmHighRisk=false`。MCP 不会真正执行写动作，但会返回带 `data.preflight` 的结构化拒绝结果，适合作为最后一道二次确认输入。
...
如果 `configPatch` 会启用 `aiReply`，或修改一个已经启用的 `aiReply` 配置，还必须额外显式传 `confirmHighRisk=true`。这是因为这类 patch 虽然本身不发送消息，但会直接改变后续自动聊天的执行边界。
...
- `chat.send`：可通过当前页面可用的 Boss 通道发消息，但 bridge / CLI / MCP 调用时必须显式传 `confirmHighRisk=true`，否则不会真正发送
- `config.update`：默认仍可更新普通运行时配置；但如果 patch 涉及启用 `aiReply`，或修改已启用的 `aiReply`，也必须显式传 `confirmHighRisk=true`
- `chat.list`：返回当前页面采集到的会话摘要；支持 `pendingReplyOnly=true` 只读筛出“最后一条消息来自 Boss、当前仍待回复”的会话，并会额外返回 `latestRole`、`needsReply`、`pendingReplyCount`、`totalConversations`
