...
- 查询命令：`useAgentJobQueries.ts`、`useAgentMetaQueries.ts`、`useAgentChatQueries.ts`
- 桥接和分发：`agentController.ts`、`agentWindowBridge.ts`、`useDeliveryControl.ts`

这层直接决定外部命令 `start`、`pause`、`resume`、`jobs.*`、`chat.*`、`config.*`、`readiness.get`、`plan.preview` 等如何落到当前页面。
...
- 监听 Boss 页面聊天 WebSocket
- 解析 protobuf 消息
- 在页面内维护聊天会话与历史
- 通过当前页面可用通道发送消息

`chat.list` 和 `chat.history` 读取的是“当前页面采集到的聊天视图”，不是 Boss 平台完整历史库。
...
- `agent-launch.mjs`: 一键启动 bridge 并打开 relay
- `agent-cli.mjs`: 命令行封装
